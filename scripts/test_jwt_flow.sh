...
resp=$(curl -sk -X POST "$GATEWAY_URL${AUTH_ENDPOINT}" -H 'Content-Type: application/json' -d '{}' || true)
echo "Raw response:" >&2
echo "$resp" >&2

# Try to extract accessToken from common response shapes
...
echo "Obtained token: ${token:0:40}..."  # show prefix only

echo
echo "2) Decoding token payload (if JWT)"
if [[ $(awk -F. '{print NF-1}' <<<"$token") -ne 2 ]]; then
  echo "Token is not a JWT (does not contain two dots)." >&2
else
