...
if [ "$COUNT" -eq 0 ]; then
    ...
    sqlite3 "$DB" "INSERT INTO monitor (user_id, name, url, type, interval, active) VALUES ($ADMIN_ID, 'PostgreSQL', 'postgres:5432', 'ping', 60, 1);"
    sqlite3 "$DB" "INSERT INTO monitor (user_id, name, url, type, interval, active) VALUES ($ADMIN_ID, 'Redis', 'redis:6379', 'ping', 60, 1);"
    ...
    sqlite3 "$DB" "INSERT INTO monitor (user_id, name, url, type, interval, active) VALUES ($ADMIN_ID, 'Prometheus', 'http://prometheus:9090', 'http', 60, 1);"
    sqlite3 "$DB" "INSERT INTO monitor (user_id, name, url, type, interval, active) VALUES ($ADMIN_ID, 'Alertmanager', 'http://alertmanager:9093', 'http', 60, 1);"
    sqlite3 "$DB" "INSERT INTO monitor (user_id, name, url, type, interval, active) VALUES ($ADMIN_ID, 'Grafana', 'http://grafana:3000', 'http', 60, 1);"
